...
class Weibo():
                ...
                for i in range(0, len(info) - 2):
                    ...
                    if weibo:
                        ...
                        if publish_time < since_date:
                            return True
                        checkweibo={}
                        if np.where(self.expected_data['微博id']==weibo['id'])[0].size==0:
                            return True
                        index=np.where(self.expected_data['微博id']==weibo['id'])[0][0]
                        checkweibo['id']=weibo['id']
                        ...
                        checkweibo['publish_place'] = self.expected_data['发布位置'][index] == weibo['publish_place']
                        checkweibo['publish_time'] = self.expected_data['发布时间'][index] == weibo['publish_time']
                        checkweibo['publish_tool'] = self.expected_data['发布工具'][index] == weibo['publish_tool']
                        # checkweibo['up_num'] = self.expected_data['点赞数'][index] == weibo['up_num']
                        # checkweibo['retweet_num'] = self.expected_data['转发数'][index] == weibo['retweet_num']
                        # checkweibo['comment_num'] = self.expected_data['评论数'][index] == weibo['comment_num']
                        self.check(checkweibo)
    ...
    def check(self,checkweibo):
        for key, value in checkweibo.items():
            if value == False:
                self.write_bug(key,checkweibo)

    def write_bug(self,key,checkweibo):
        file_dir = os.path.split(
            os.path.realpath(__file__))[0] + os.sep
        file_path = file_dir + 'log.txt'
        content = u'微博id=%s中信息：%s出现问题\n' % (checkweibo['id'],key)
        print(content)
    ...
    def start(self):
        """获得expected_result"""
        self.expected_data = pd.read_csv('./weibo/一沾枕头就能着/6401973058.csv')
        """运行爬虫"""
        try:
